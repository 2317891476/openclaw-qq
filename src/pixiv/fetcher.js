...
export async function fetchByParsed(client, parsed) {
 ...
 stageBuckets.set(stage.label, bucket);
 ...
 if (parsed.range) {
 selected = rankedIds.slice(Math.max(0, parsed.range.start - 1), parsed.range.end);
 // fallback pool: when selected items fail to download/filter, keep filling from remaining candidates
 fallbackPool = rankedIds.slice(parsed.range.end);
 } else if (mode === 'users' && !parsed.tierMix) {
 // Priority random for users mode:
 // random within 10000 pool first; if insufficient, descend to 5000 -> 1000 -> base.
 const picked = new Set();
 const priorityLabels = parsed.noHq ? ['base'] : ['10000', '5000', '1000', 'base'];
 for (const label of priorityLabels) {
 const pool = shuffle(stageBuckets.get(label) || []);
 ...
 if (selected.length < parsed.count) {
 ...
 const pool = shuffle(stageBuckets.get(label) || []);
 for (const id of pool) {
 if (!picked.has(id)) remains.push(id);
 }
 }
 // Include any leftovers that were not captured in stage buckets (defensive).
 for (const id of rankedIds) {
 if (!picked.has(id) && !remains.includes(id)) remains.push(id);
 }
 fallbackPool = remains;
 } else {
 selected = shuffle(rankedIds).slice(0, parsed.count);
 // fallback pool: when selected items fail to download/filter, keep filling from remaining candidates
 fallbackPool = rankedIds;
 }

 const qualityNote = parsed.noHq ? '（nohq）' : `（分层:${stageStats.join(' > ')}）`;
 const filterNote = `（mode=${mode}${parsed.tierMix ? ',tierMix=on' : ''}${parsed.minBookmark ? `,minBk=${parsed.minBookmark}` : ''}${parsed.ratio ? `,ratio=${parsed.ratio}` : ''}）`;
 ...
 const meta = await client.illustMeta(id).catch(() => null);
 if (!meta) continue;
 scored.push({ id: String(id), bookmarkCount: Number(meta.bookmarkCount || 0), ts: meta.createDate ? Date.parse(meta.createDate) : 0 });
 }
