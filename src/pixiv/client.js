...
function normalizeAuthCookie(cfg = {}) {
 const raw = String(cfg.cookie || '').trim();
 if (raw) return raw;
 const phpsessid = String(cfg.phpSessId || '').trim();
 if (phpsessid) return `PHPSESSID=${phpsessid}`;
 return '';
}

function headers(referer, authCookie = '') {
 return {
 'User-Agent': UA,
...
export class PixivClient {
 constructor(cfg = {}) {
 this.authCookie = normalizeAuthCookie(cfg);
 }
 ...
 const r2 = await fetch(tu, { headers: { 'User-Agent': UA, Referer: 'https://www.pixiv.net/', ...(this.authCookie ? { Cookie: this.authCookie } : {}) }, signal: AbortSignal.timeout(3500) });
 if (r2.ok) {
 ...
 const r = await fetch(url, { headers: headers(`https://www.pixiv.net/tags/${encodeURIComponent(keyword)}/artworks?s_mode=s_tag`, this.authCookie) });
 if (!r.ok) continue;
 ...
 const r = await fetch(url, { headers: headers(`https://www.pixiv.net/search/users/${encodeURIComponent(q)}`, this.authCookie) });
 if (r.ok) {
 const j = await r.json();
 ...
 const ar = await fetch(aurl, { headers: headers(`https://www.pixiv.net/tags/${encodeURIComponent(q)}/artworks?s_mode=s_tag`, this.authCookie) });
 if (!ar.ok) return [];
 ...
 const r = await fetch(url, { headers: headers(`https://www.pixiv.net/users/${encodeURIComponent(userId)}`, this.authCookie) });
 if (!r.ok) return [];
 const j = await r.json();
 ...
 const r = await fetch(url, { headers: headers(`https://www.pixiv.net/ranking.php?mode=${encodeURIComponent(mode)}`, this.authCookie) });
 if (!r.ok) return [];
 const j = await r.json();
 ...
 const dr = await fetch(detailUrl, { headers: headers(`https://www.pixiv.net/artworks/${id}`, this.authCookie) });
 ...
 const pr = await fetch(pagesUrl, { headers: headers(`https://www.pixiv.net/artworks/${id}`, this.authCookie) });
 ...
 const r = await fetch(meta.original, { headers: { 'User-Agent': UA, Referer: `https://www.pixiv.net/artworks/${meta.id}`, ...(this.authCookie ? { Cookie: this.authCookie } : {}) } });
 if (!r.ok) return null;
