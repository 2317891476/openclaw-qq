export function parsePixivCommand(cmdText) {
 ...
 const tierMix = args.includes('--tier_mix') || args.includes('--tier-mix');
 ...
 if (['--nsfw', '--nohq', '--no-hq', '--alltime', '--verbose', '--count_first', '--count-first', '--quality_first', '--quality-first', '--tier_mix', '--tier-mix'].includes(x)) return false;
